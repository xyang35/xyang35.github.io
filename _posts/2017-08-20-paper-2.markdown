...
从几何上看，上文的大气散射模型说明了在 RGB 颜色空间当中，向量\\( A \\)、\\( I(x) \\)和\\( J(x) \\)是共面的，而且它们的终点应该是共线的，透射率\\( t(x) \\)是\\( \left\|A-I(x)\right\| \\)和\\( \|A-J(x)\| \\)的比值。如下图：
...
另外从公式中我们可以看到，当距离趋近于无穷大，即\\( d(x) \\) -> \\( inf \\)时，我们有\\( A=I(x) \\)。虽然实际中距离不可能是无穷大的，但是我们可以利用这一性质来估计大气的全局光照，即找到透射率最小（雾气最浓）的位置的最大光照强度：\\( A = \max_{y\in \left\{x\|t(x)\leq t_0 \right\}}I(y) \\)。
