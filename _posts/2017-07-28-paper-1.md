...
这篇文章是当天早上的第一个oral presentation，也是我觉得最有趣的一个（之后有两个都是best paper的presentation......）。文章要解决的是数据标签中含有噪声的问题，即部分数据的标签由于噪声的影响变成错误的标签。用数学语言来表达就是，对于一个数据集，我们可以将数据及其标签的联合分布表示为 \\(p(x, \tilde{y}) = \sum_y p(\tilde{y}|y)p(y|x)p(x)\\)
